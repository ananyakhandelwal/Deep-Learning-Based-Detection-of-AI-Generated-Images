...
class RealVsAIClassifier:
    ...
    def create_model(self):
        ...
        # InceptionV3 specific preprocessing (expects 0-255 inputs, scales to -1 to 1)
    ...
    def prepare_data_generators(self, train_dir):
        ...
        train_datagen = ImageDataGenerator(
            ...
            # Note: No rescale needed for InceptionV3 preprocess_input
        )
    ...
    def unfreeze_and_finetune(self, train_gen, val_gen):
        ...
        # InceptionV3 has 311 layers. We'll freeze the first 249 layers (roughly 80%)
        # and fine-tune the last ~60 layers (the last two inception blocks)
        # Layers after 'mixed7' are good candidates for fine-tuning
        for layer in self.base_model.layers[:249]:
            layer.trainable = False
        for layer in self.base_model.layers[249:]:
            layer.trainable = True

        print(f"Fine-tuning {sum([layer.trainable for layer in self.base_model.layers])} layers")
    ...
    def train(self, train_gen, val_gen, epochs=15, is_finetuning=False):
        ...
        checkpoint_name = 'InceptionV3_best_model_finetuned.keras' if is_finetuning else 'InceptionV3_best_model.keras'
    ...
    def plot_results(self, test_gen):
        ...
        ax[1].legend()
        plt.savefig("/kaggle/working/InceptionV3_accuracy_loss.png", dpi=300, bbox_inches='tight')
        plt.show()
        ...
        ax[1].legend()
        plt.savefig("/kaggle/working/InceptionV3_roc_pr.png", dpi=300, bbox_inches='tight')
        plt.show()

    def save_final(self):
        self.model.save('/kaggle/working/InceptionV3_cifake_final.keras')
        print("✓ Model saved as InceptionV3_cifake_final.keras")
        self.model.save('/kaggle/working/InceptionV3_cifake_final.h5')
        print("✓ Model saved as InceptionV3_cifake_final.h5")
...
def main():
    ...
    # Initialize - Note: InceptionV3 uses 299x299 input size
    classifier = RealVsAIClassifier(img_size=(299, 299), batch_size=32)
